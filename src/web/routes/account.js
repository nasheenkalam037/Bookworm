...
sql_user_reviews =
  'SELECT r.*, b.title from "Reviews" as r INNER JOIN "Books" b ON r.book_id = b.book_id where r.user_id = $1';
// Private account
router.get('/me', async function(req, res, next) {
  var myuser = user.getUser(req.session);
  var reviews = null;
  if (myuser) {
    var user_id = myuser.user_id;
    var { rows } = await db.query(sql_user_reviews, [user_id]);
    reviews = rows;
  }
  ...
  res.render('private_account', {
    title: 'The Bookworm My Account Page',
    user: myuser,
    reviews: reviews
  });
});
...
router.post('/signup', async function(req, res, next) {
  var error = undefined;
  var p = req.body;
  if ('fullname' in p && 'email' in p && 'password' in p && 'confirmation_password' in p) {
    if (p.fullname.length > 1 && p.email.length > 1 && p.password.length > 1) {
      if (p.password == p.confirmation_password) {
        ...
        try {
          var { rows } = await db.query(sql_insert_new_user, [p.fullname, p.email, hash]);
